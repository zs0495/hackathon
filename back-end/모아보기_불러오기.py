...
# 4. 모든 키워드 통합 (중복 제거)
 ...
 # 5. DB에서 복지 혜택 검색
  ...
  SELECT DISTINCT 
                ...
                b.eligibility,
                b.appl_method,
                b.required_doc AS required_documents,
                b.ben_url AS link,
                s.site_name,
                b.is_nationwide
            FROM benefits b
  ...
  # JOIN 조건 추가 (지역 또는 전국 대상)
    ...
    # 공공/민간 구분 (site_id 범위)
        if welfare_type == 'PUBLIC':
            query += " AND b.site_id BETWEEN 1 AND 7"
        else:  # PRIVATE
            query += " AND b.site_id BETWEEN 8 AND 12"
        
        # 지역 조건 (city 기준, 전국 대상 포함)
        if city:
        ...
        # 키워드 조건 (OR 조건으로 검색)
        if all_keywords:
            keyword_placeholders = ','.join(['%s'] * len(all_keywords))
            query += f" AND k.keyword_content IN ({keyword_placeholders})"
        ...
        for row in results:
            matched_benefits.append({
                ...
                'required_documents': row['required_documents'].split(',') if row['required_documents'] else [],
                'link': row['link'],
